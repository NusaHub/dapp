...
const VerificationPage = () => {
    ...
    const handleConfirmVerification = async () => {
        ...
        setIsLoading(true);
        try {
            await addVerifiedUser(verificationResult.vaultid);
            toast.success("Identity Confirmed!", { description: "You are now a verified user." });
            router.push('/game-projects');
        } catch (error) {
            toast.error("Confirmation Failed", { description: "Could not save verification data." });
        } finally {
            setIsLoading(false);
            setCapturedIdentity(null);
            setCapturedFace(null);
            ...
            <Dialog open={isResultDialogOpen} onOpenChange={setIsResultDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Please confirm if the details below are correct.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-2 text-sm">
                        <p><strong>Name:</strong> {verificationResult?.fullName}</p>
                        <p><strong>Date of Birth:</strong> {verificationResult?.dob}</p>
                        <p><strong>Nationality:</strong> {verificationResult?.nationality_full}</p>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={handleReverify}>Re-verify</Button>
