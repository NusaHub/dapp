...
import { proposeProgress } from "@/services/governor";
...
const MilestoneAccordion = ({ milestones, projectId, paymentToken, onMilestoneUpdate }: MilestoneAccordionProps) => {
    ...
    const handleApproval = async (milestoneId: string) => {
        ...
        try {
            ...
            if (approvalResult) {
                ...
                toast.success(`${paymentToken === 0 ? "IDRX" : "USDT"} tokens approved successfully!`);
    ...
    const handleSaveOutput = async (milestoneId: string) => {
        ...
        try {
            ...
            const proposalId = await proposeProgress(formValues.description, projectId);
            console.log("✅ [Save] Proposal created:", { proposalId, description: formValues.description });
            ...
            const amount = outputType === "monetary" && formValues.amount ? formValues.amount : 0;
            ...
            if (result) {
                toast.success(`Milestone output saved successfully! Proposal ID: ${proposalId}`);
                                ...
                                {milestone.outputDescription ? (
                                    <div>
                                        ...
                                        {milestone.outputType === "monetary" && (
                                            <div className="mt-2 text-sm space-y-1">
                                                {
                                                    paymentToken === 0 ? (
                                                        <p><strong>IDRX:</strong> {milestone.output || 'N/A'}</p>
                                                    ) : (
                                                        <p><strong>USDT:</strong> {milestone.output || 'N/A'}</p>
                                                    )
                                                }
                                            </div>
                                        )}
                                    ...
                                    >
                                        ...
                                        <Textarea
                                            ...
                                            onChange={(e) => handleInputChange(milestone.id, "description", e.target.value)}
                                        />

                                        {outputTypes[milestone.id] === "monetary" && (
                                            <div className="space-y-4">
                                                <Label className="text-sm font-medium">
                                                    Amount ({paymentToken === 0 ? "IDRX" : "USDT"} - will be converted automatically):
                                                </Label>
                                                <Input
                                                    type="number"
                                                ...
                                                <div className="p-3 bg-purple-50 border border-purple-200 rounded-md">
                                                    <p className="text-sm text-purple-700">
                                                        Before saving, you need to approve {paymentToken === 0 ? "IDRX" : "USDT"} tokens for this transaction.
                                                    </p>
                                                </div>
                                            </div>
                                        )}

                                        {outputTypes[milestone.id] === "monetary" && !approvalStates[milestone.id] ? (
                                            <Button
                                                size="sm"
                                                type="button"
                                                onClick={() => handleApproval(milestone.id)}
                                                disabled={approvingStates[milestone.id] || !formData[milestone.id]?.amount}
                                            ...
                                            >
                                                {approvingStates[milestone.id] ? (
                                                    ...
                                                    <>
                                                        Approve {paymentToken === 0 ? "IDRX" : "USDT"} Tokens
                                                    </>
                                                )}
                                            </Button>
                                        ) : (
                                            <div className="space-y-2">
                                                {approvalStates[milestone.id] && (
                                                    <div className="flex items-center gap-2 text-green-600 text-sm">
                                                        ...
                                                        {paymentToken === 0 ? "IDRX" : "USDT"} tokens approved successfully
                                                    </div>
                                                )}
                                                <Button
                                                    size="sm"
